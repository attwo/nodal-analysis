...
def _make_line_repr(source: dict):
    return LineString([(round(source['q_liq'][i], 2), round(source['p_wf'][i], 2)) for i in range(len(source['q_liq']))])
...
def calc_nodal(vlp: dict, ipr: dict):
    ...
    return [{'q_liq': inter[0], 'p_wf': inter[1]} for inter in intersection.coords]
